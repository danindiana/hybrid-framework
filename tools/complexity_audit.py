...
def audit_directory(path):
    ...
    for root, dirs, files in os.walk(path):
        # Skip dependency, cache, and hidden directories
        dirs[:] = [d for d in dirs if d not in ('venv', '.git', '__pycache__', '.antigravitycli')]
        ...
        for file in files:
            if file.endswith(('.py', '.sh', '.js', '.ts', '.md')):
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to audit {file_path}: {str(e)}")
